...
class EposControlNode(Node):
    def __init__(self):
        ...
        # hand_norm: [index, middle, ring, pinky] の想定
        self.create_subscription(
            Float32MultiArray, "/hand_norm", self.hand_norm_callback, 10
        )
        # slider_raw: Int32（0〜680）
        self.create_subscription(
            Int32, "/slider_raw", self.slider_callback, 10
        )
    ...
    def hand_norm_callback(self, msg: Float32MultiArray):
        if not msg.data or len(msg.data) < 1:
            ...
            return
        # 配列の 0 番目を人差し指として利用
        self.norm_avg = float(msg.data[0])
